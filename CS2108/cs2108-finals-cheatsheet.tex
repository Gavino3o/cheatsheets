...
\begin{multicols*}{4}
  ...
  \begin{itemize}
    ...
    \begin{equation}
      y(t) = A \sin(\omega t + \phi) \hspace{5px} \text{or} \hspace{5px} y(t) = B \cos(\omega t + \phi)
    \end{equation}
    where:
    \begin{itemize}
      \item $A$ and $B$ are the amplitudes
    ...
    \begin{equation}
      y(t) = A \sin(2\pi f t + \phi) \hspace{5px} \text{or} \hspace{5px} y(t) = B \cos(2\pi f t + \phi)
    \end{equation}
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{equation}
        ...
        \end{aligned}
      \end{equation}
      \item If $a_k$ = $b_k$, the sinusoid is a combination of sine and cosine functions with equal amplitude and phase shift. Phase shift is 45 degrees. $c_k = \sqrt{2}a_k$
      \item If $a_k$ = -$b_k$, the sinusoid is a combination of sine and cosine functions with equal amplitude and phase shift. Phase shift is -45 degrees. $c_k = \sqrt{2}a_k$
      \item If $a_k$ = 0, phase shift is 0 degrees
      \item If $b_k$ = 0, phase shift is 90 degrees
    \end{itemize}
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item If we add sinusoids with different frequencies, the resulting function is not periodic unless the frequencies are rationally related.
    ...
    \item The above calculations can be done with sinusoids of more than 2 frequencies as well.
 ...
 \section{\underline{Fourier Series}}
  ...
  \begin{itemize}
    ...
    \item \textbf{Sine and cosine functions make up the basis functions}. I.e for a sinusoidal signal with highest frequency $f_m$, we can represent it as a sum of $M$ sine and cosine functions with frequencies $f_0, f_1, \ldots, f_{M-1}$.
    \begin{equation}
  ...
  \begin{itemize}
    ...
    \item \textbf{Note}: Sometimes, we pad the input signal with zeros to increase the number of samples. Padding zeros to the input decreases the frequency bin size, increasing the frequency resolution. But it does not increase the actual frequency content of the signal.
  ...
  \begin{itemize}
    ...
    \item If you shift a signal $x[n]$ by $n_0$ samples in time, the Fourier transform $X[k]$ remains the same in magnitude, but is multiplied by a complex exponential.
  ...
  \begin{itemize}
    ...
    \includegraphics[width=0.75\linewidth, height=100px]{dft_sine_pair.png}
  ...
  \begin{itemize}
    ...
    \includegraphics[width=0.85\linewidth, height=100px]{line_pair_sampling_freq.png}
    ...
    \begin{itemize}
      ...
      \end{equation}
      \item Derivation of the convolution theorem (continuous version):
      \includegraphics[width=0.95\linewidth]{convolution_continuous_linear.png}
    ...
    \begin{itemize}
      ...
      \item For better performance, we can \textbf{pad the input image and kernel to the next power of 2}. This is because the FFT algorithm is based on the divide-and-conquer approach, which works best when the number of samples is a power of 2.
    \end{itemize}

    \section{\underline{Sampling Theorem}}
    \begin{itemize}
      \item Fun fact: 44100 Hz is the standard sampling frequency for audio signals. This is because it is twice the highest frequency that can be heard by humans, which is 20 kHz.
      \item Recall that the the Fourier transform of an impulse train is another impulse train.
      \includegraphics[width=0.95\linewidth, height=100px]{dft_impulse_train_fs.png}
    ...
    \begin{itemize}
      \item Sacrifice part of the signal least critical to human perception.
      \item Image: Missing crispness, colour, and detail.
    ...
    \textbf{Steps:}
    \begin{itemize}
      \item \textbf{Convert RGB to YCbCr colour space}. Y is the luminance (brightness) component, and Cb and Cr are the chrominance (colour) components.
      \item Downsample the chrominance components (Cb and Cr) by a factor of 2. This is done because the human eye is less sensitive to colour than brightness.
    ...
    \begin{itemize}
      \item RLE is a simple form of \textbf{lossless} data compression. It replaces consecutive repeated values with a single value and a count of how many times it is repeated.
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \begin{itemize}
          \item Retrieve the two nodes with the lowest frequency.
    ...
    \begin{itemize}
      ...
      \item \textbf{Encodes 8 bit data using n bit codes}, where $n > 8$. 0-255 are the original 8 bit data, and 256-4095 are the new codes and are data dependent.
      ...
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          \item Add W + K to the dictionary with the next available code number.
      ...
      \section{\underline{Notes to self}}
      \begin{itemize}
        \item When you perform FFT on a signal $y[n]$, the result $Y[k]$ is complex valued. 
        \begin{equation}
          Y[k] = | Y[k] | e^{j \phi_k}
        \end{equation}
        where: 
        \begin{itemize}
          \item $| Y[k] |$ is the magnitude
          \item $\phi_k$ is the phase of frequency bin $k$
        \end{itemize}
        \item When you take the \textbf{magnitude of the FFT} result, you are \textbf{zeroing out all phase information}. i.e $\phi_k = 0$.
        \item All sinusoid waves become pure cosines when phase is lost, because cosine is the default phase-zero sinusoid. 
        \item Sine wave: $\sin(2 \pi ft) = \cos(2 \pi ft - \pi/2)$
      \end{itemize}
