...
\pdfinfo{
  ...
  /Author (Jovyn Tan and Gavin Chiam)
  /Subject (CS3223)
...
\begin{multicols*}{4}
  ...
  \begin{center}
    \fbox{%
      \parbox{0.8\linewidth}{\centering \textcolor{black}{
          {\Large\textbf{CS3223 AY23/24 SEM 2}}}
      }%
    }
  \end{center}

  \vspace{-0.4cm}
  \includegraphics[height=24mm,width=0.9\linewidth]{cs3223-notation.png} 
  \vspace{-0.2cm}
  ...
  \begin{itemize}
    \item \definition{conflict serialisable} conflict equivalent to a serial sch
      \begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item \textbf{write-ahead logging (WAL)}: do not flush uncommitted update to DB until log record containing before-image is flushed to log \textbf{(abort)}
    \item \textbf{force-at-commit}: do not commit txn until the after-images of all its updated records are in stable storage \textbf{(commit)}
