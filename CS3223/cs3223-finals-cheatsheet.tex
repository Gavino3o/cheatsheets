...
\begin{multicols*}{4}
  ...
  \begin{itemize}
    ...
    \item \definition[protocol]{write-ahead logging (WAL)} do not flush uncommitted update to DB until log record containing before-image is flushed to log \textbf{(abort)}
    \item \definition[protocol]{force-at-commit} do not commit txn until the after-images of all its updated records are in stable storage \textbf{(commit)}
