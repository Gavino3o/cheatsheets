...
\begin{multicols*}{4}
  ...
  \begin{itemize}
    ...
    \item\textbf{n-ary relationships:} relationship sets that connect more than 2 entity sets. All entities must be participating, not one can be missing.
    \includegraphics[width=0.97\linewidth]{er2.png} 
    ...
    \includegraphics[width=0.97\linewidth]{er3.png} 
    ...
    \includegraphics[width=0.97\linewidth]{er4.png} 
    \item \textbf{Participation Contraints}: Between entity and relationship set. Restricted by (minimum, maximum) cardinality.
    \textbf{Default: (0, n)}
    \includegraphics[width=0.97\linewidth]{participation_contraint.png}
    \item Weak entities can only be defined for a (1, 1) constraint. 
    \item \includegraphics[width=0.7\linewidth, height = 150px]{er5.png} 
    ...
    \begin{itemize}
      ...
      \item \textbf{Rule 2}: Map each entity set to a relation. Canidate keys are mapped to primary keys and UNIQUE NOT NULL.
      \item Composite keys can be specificied using table constraints. (PRIMARY KEY (attr1, attr2))
  ...
  \begin{itemize}
    \item \textbf{OLTP:} Online Transaction Processing. Focuses on handling a large number of transactions, with a small amount of data per transaction, data up-do-date and consistent, concurrency. (e.g. banking, e-commerce)
    ...
    \item Kimballs's Principle for Data Warehousing: Consistency, Accessbility, Security, Adaptability, Acceptance, Decision Support.
  ...
  \begin{itemize}
    ...
    \item Planner/Optimizer: Generates query plans using catalouge and statistics, choose plan with least estimated cost.
  ...
  \begin{itemize}
    ...
    \item \textbf{Bitmap Index Scan}: Uses a bitmap index to retrieve the row locations of the matching rows. Can be used for conjuctions or disjunctions of predicates on indexed columns.
  \end{itemize}
